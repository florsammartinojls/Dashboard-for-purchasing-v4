...
import { batchProfiles, calcCoverageNeed, fillToMOQ as fillToMOQCalc, getCalcBreakdown, DEFAULT_PROFILE } from "../lib/seasonal";
...
export default function PurchTab({ data, stg, goCore, goBundle, goVendor, ov, setOv, initV, clearIV, saveWorkflow, deleteWorkflow, saveVendorComment }) {
  ...
  // === SEASONAL PROFILES (pre-computed for all cores) ===
  const profiles = useMemo(() => {
    return batchProfiles(data.cores || [], data._coreInv || [], data._coreDays || []);
  }, [data.cores, data._coreInv, data._coreDays]);

  // === ENRICHED CORES (with seasonal need) ===
  const enr = useMemo(() => (data.cores || [])
    // BUG FIX: filter out JLS entries that aren't cores
    ...
    .filter(c => {
      ...
      const ai = cAI(c);
      // Seasonal coverage need
      const profile = profiles[c.id] || DEFAULT_PROFILE;
      const coverage = calcCoverageNeed(c, lt, tg, profile);
      const sNeed = coverage.need;
      const flatNeed = cNQ(c, tg);
      const oq = cOQ(sNeed, c.moq, c.casePack);
      const seas = cSeas(c.id, (data._coreInv || []));
      return { ...c, status: st, allIn: ai, needQty: sNeed, flatNeed, orderQty: oq, needDollar: +(oq * c.cost).toFixed(2), docAfter: cDA(c, oq), lt, critDays: cd, warnDays: wd, targetDoc: tg, vc: v.country || "", seas, isDom: isD(v.country), spike: c.d7 > 0 && c.dsr > 0 && c.d7 >= c.dsr * 1.25, sProfile: profile, sCoverage: coverage };
    }).filter(c => {
      ...
      return 0;
    }), [data, stg, vf, sf, sort, vMap, nf, minD, locF, profiles]);
  ...
  const rawAllocMap = useMemo(() => {
    ...
    enr.forEach(c => {
      ...
      const bData = bundles.map(b => {
        ...
        const baseDOC = dsr > 0 ? inv / dsr : 9999; const qtyPerBundle = b.qty1 || 1;
      ...
      bData.forEach(bd => {
        ...
        const needDOC = Math.max(0, tg - bd.baseDOC); const needBU = needDOC * bd.dsr;
        const needCP = needBU * bd.qtyPerBundle; const give = Math.min(needCP, pool); pool -= give;
  ...
  // === FILL REC (now uses seasonal need for cores) ===
  const fillR = (cores, bundles, mode, vendorName) => {
    ...
    if (mode === "bundles") {
      ...
      (bundles || []).forEach(b => {
        ...
        if (need <= 0) return;
        const qpb = b.qty1 || 1;
        if (bMoq > 0 && need < bMoq) { coreExtrasFromBundles[pc.id] = (coreExtrasFromBundles[pc.id] || 0) + (need * qpb) }
        else { u[b.j] = { ...(u[b.j] || {}), pcs: bMoq > 0 ? Math.max(need, bMoq) : need } }
      });
      cores.forEach(c => {
      ...
      (bundles || []).forEach(b => {
        ...
        if (need < effectiveMoq) { coreExtras[pc.id] = (coreExtras[pc.id] || 0) + (need * qpb) }
        else {
          let ord = bMoq > 0 ? Math.max(need, bMoq) : need;
          const bcp = casePackFromRec[b.j] || 0;
          if (bcp > 0) ord = Math.ceil(ord / bcp) * bcp;
          u[b.j] = { ...(u[b.j] || {}), pcs: ord }; cwo.add(pc.id);
        }
      });
      cores.forEach(c => {
        const extra = coreExtras[c.id] || 0;
        const raw20d = Math.ceil((c.dsr || 0) * 20);
        // Use seasonal need (c.needQty is already seasonal)
        if (extra > 0) { u[c.id] = { ...(u[c.id] || {}), pcs: cOQ(Math.max(extra, raw20d), c.moq, c.casePack) } }
        else if (!cwo.has(c.id) && c.needQty > 0) { u[c.id] = { ...(u[c.id] || {}), pcs: cOQ(Math.max(c.needQty, raw20d), c.moq, c.casePack) } }
      });
    } else {
      // CORES mode — uses seasonal needQty
  ...
  const doFillMOQ = (grpCores, grpBundles, vendorMOQDollar) => {
    ...
    if (vendorSub !== "cores") {
      (grpBundles || []).filter(b => hasBundleOrd(b)).forEach(b => {
        const f = feMap[b.j]; currentTotal += bundleEffQ(b) * (f?.aicogs || 0);
      });
    }
    if (currentTotal >= vendorMOQDollar) { setToast("Already at/above MOQ"); return }
    const lt = grpCores[0]?.lt || 30;
    const tg = grpCores[0]?.targetDoc || 90;
    ...
    Object.entries(extra).forEach(([id, extraPcs]) => {
      ...
      if (c) addedTotal += extraPcs * c.cost;
    });
    setOv(u);
    setToast("MOQ filled: +" + $(addedTotal));
  };
  ...
  const openBreakdown = useCallback((c) => {
    ...
    const tg = gTD(v, stg);
    const profile = profiles[c.id] || DEFAULT_PROFILE;
    setBreakdown(getCalcBreakdown(c, v, stg, profile, lt, tg));
  }, [vMap, stg, profiles]);
  ...
  const CoreRow = ({ c }) => {
    ...
    if (eq > 0 && c.dsr > 0) {
      if (activeBundlesForCore.length === 1) {
        const sb = activeBundlesForCore[0]; const alloc = rawAllocMap[sb.j] || { rawUnits: 0, baseInv: 0 };
        const bOrd = bundleEffQ(sb);
        const bInv = alloc.baseInv + alloc.rawUnits + bOrd + (eq / (sb.qty1 || 1));
  ...
  const BundleRow = ({ b }) => {
    ...
    const inb7f = missingMap[b.j] || 0; const rp = replenMap[b.j];
    ...
    return <tr className={`border-t border-gray-800/20 hover:bg-indigo-900/10 text-xs ${hasBundleOrd(b) ? "bg-emerald-900/10" : "bg-indigo-950/20"}`}>
      ...
      <td className="py-1 px-1 text-indigo-200 truncate max-w-[100px] sticky left-24 bg-indigo-950/20 z-10">
        {b.t}{b.asin && <a href={`https://sellercentral.amazon.com/myinventory/inventory?fulfilledBy=all&page=1&pageSize=25&searchField=all&searchTerm=${b.asin}&sort=date_created_desc&status=all`} target="_blank" rel="noopener noreferrer" className="ml-1 text-gray-500 hover:text-blue-400 text-[9px] font-mono" title="Open in Seller Central">{b.asin}</a>}
  ...
  const VTH = ({ isCol }) => <tr className="text-gray-500 uppercase bg-gray-900 text-xs sticky top-0 z-20">
    ...
    {!isCol && <><TH tip="Raw / Potential Units" className="py-2 px-1 text-right">Raw</TH><TH tip="PPRC Available" className="py-2 px-1 text-right">PPRC</TH><TH tip="Inbound Pieces" className="py-2 px-1 text-right">Inb</TH><TH tip="Total FBA & Inbound Pieces" className="py-2 px-1 text-right">FBA Pcs</TH></>}
    {showRS && <><th className="py-2 border-l-2 border-cyan-800 px-0.5" /><TH tip="Bundle FIB DOC" className="py-2 px-1 text-right text-cyan-400">FibDoc</TH><TH tip="Margin %" className="py-2 px-1 text-right text-cyan-400">Mrgn</TH><TH tip="Raw Units (Replen)" className="py-2 px-1 text-right text-cyan-400">Raw</TH><TH tip="Batched" className="py-2 px-1 text-right text-cyan-400">Batch</TH><TH tip="FIB Inventory" className="py-2 px-1 text-right text-cyan-400">FIB</TH><TH tip="PPRC Units" className="py-2 px-1 text-right text-cyan-400">PPRC</TH><TH tip="7f Missing" className="py-2 px-1 text-right text-red-400">7f Miss</TH></>}
    ...
    {showCosts && <><TH tip="Inbound Shipping" className="py-2 px-1 text-center">InbS</TH><TH tip="Cost per Piece" className="py-2 px-1 text-center">CogP</TH><TH tip="Cost per Case" className="py-2 px-1 text-center">CogC</TH></>}
      ...
      return <div key={v.name} className="mb-5 border border-gray-800 rounded-xl overflow-hidden">
        <div className="bg-gray-900 px-4 py-3">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            {/* FILL TO MOQ button */}
            <button onClick={() => doFillMOQ(grp.cores, grp.bundles, v.moqDollar || 0)} disabled={!v.moqDollar || poT >= v.moqDollar || poI.length === 0} className={`text-xs px-2.5 py-1 rounded font-medium ${v.moqDollar && poT < v.moqDollar && poI.length > 0 ? "bg-orange-600 text-white" : "bg-gray-700 text-gray-500 cursor-not-allowed"}`} title="Add extra to reach vendor MOQ">Fill MOQ{moqGap > 0 && poI.length > 0 ? ` (+${$(moqGap)})` : ""}</button>
