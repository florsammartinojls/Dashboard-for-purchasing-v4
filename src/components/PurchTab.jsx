  ...
  const BundleRow = ({ b }) => {
    ...
    return <tr className={`border-b border-gray-800/20 hover:bg-indigo-900/20 text-xs ${rowBg}`}>
      ...
      <td className={`py-1.5 px-1 text-indigo-200 max-w-[180px] sticky left-[85px] z-10 ${stickyBg}`}>
        ...
        {bd?.bundleMoqStatus === 'bdlmoq-skipped-no-need' && <span className="ml-1 text-[10px] font-semibold text-gray-300 bg-gray-700/40 border border-gray-700 px-1.5 py-0.5 rounded" title="BdlMOQ defined but bundle has no need today. Skipped — won't be in this PO.">○ MOQ skipped</span>}
        {bd?.bundleMoqStatus === 'bdlmoq-need-below-moq' && <span className="ml-1 text-[10px] font-semibold text-amber-200 bg-amber-500/20 border border-amber-500/50 px-1.5 py-0.5 rounded" title={`Need (${bd.bundleMoqOriginalNeed}) is below the BdlMOQ. Recommender did NOT auto-apply the MOQ — open Why Buy 📊 to choose: buy MOQ anyway, switch to core mode, or throttle demand.`}>⚠ MOQ review</span>}
