...
export default function BundleTab({ data, stg, hist, daily, bundleId, onBack, goCore }) {
  ...
  if (!b) return <div className="p-4 max-w-5xl mx-auto">
    <div className="flex items-center gap-3 mb-4">
      ...
      <input type="text" placeholder="Search bundle..." value={s} onChange={e => setS(e.target.value)} className="bg-gray-800 border border-gray-700 text-white rounded-lg px-4 py-2.5 flex-1 max-w-md text-sm" />
    </div>
    {s.length >= 2 ? <div className="space-y-1">{(data.bundles || []).filter(x => { const q = s.toLowerCase(); return x.j.toLowerCase().includes(q) || x.t.toLowerCase().includes(q) }).slice(0, 12).map(x => {
      ...
      {abcA.length > 0 && <div className="mt-4">
        ...
        <div className="overflow-x-auto"><table className="w-full text-xs"><thead><tr className="text-gray-500 uppercase"><th className="py-1 px-2 w-6" /><th className="py-1 px-2 text-left">JLS</th><th className="py-1 px-2 text-left">Title</th><th className="py-1 px-2 text-right">Revenue</th><th className="py-1 px-2 text-right">Profit</th><th className="py-1 px-2 text-right">Units</th><th className="py-1 px-2 text-center">ABC</th></tr></thead>
          <tbody>{abcSorted.slice(0, 50).map((a, i) => {
  ...
  return <div className="p-4 max-w-7xl mx-auto">
    ...
    <div className="bg-gray-900 rounded-xl p-4 mb-4 border border-gray-800">
      <div className="flex flex-wrap items-center gap-3 mb-2">
        ...
        {bAged && <HealthBadge health={bAged.fbaHealth} ltsf={bAged.storageLtsf} />}
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
      ...
      <div className="bg-gray-900 rounded-xl p-4 border border-gray-800"><h4 className="text-gray-500 text-xs uppercase mb-3">Profitability</h4><div className="grid grid-cols-3 gap-y-4">{[{ l: "Price", v: fee?.pr }, { l: "COGS", v: fee?.pdmtCogs }, { l: "AICOGS", v: fee?.aicogs }, { l: "Fee", v: fee?.totalFee }, { l: "GP", v: fee?.gp, c: "text-emerald-400" }, { l: "Net", v: fee?.netRev }].map(k => <div key={k.l}><div className="text-gray-500 text-xs">{k.l}</div><div className={`font-bold text-lg ${k.c || "text-white"}`}>{k.v != null ? $2(k.v) : "—"}</div></div>)}</div></div>
    </div>
    {/* Revenue Table */}
    ...
    {/* YoY Units — BAR chart */}
    {sHF.length > 0 && <div className="bg-gray-900 rounded-xl p-4 mb-4 border border-gray-800"><h3 className="text-white font-semibold text-sm mb-2">YoY Units <span className="text-gray-500 text-xs font-normal">excl. current</span></h3><ResponsiveContainer width="100%" height={200}><BarChart data={yD}><CartesianGrid strokeDasharray="3 3" stroke="#374151" /><XAxis dataKey="month" tick={{ fill: "#9ca3af", fontSize: 10 }} /><YAxis tick={{ fill: "#9ca3af", fontSize: 10 }} /><Tooltip {...TTP} /><Legend />{yrs.map(y => <Bar key={y} dataKey={"u_" + y} fill={YC[y] || "#6b7280"} opacity={0.85} radius={[2, 2, 0, 0]} name={"" + y} />)}</BarChart></ResponsiveContainer></div>}
  </div>;
}
