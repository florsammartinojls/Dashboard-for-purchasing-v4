...
export default function CoreTab({ data, stg, hist, daily, coreId, onBack, goBundle }) {
  ...
  return <div className="p-4 max-w-7xl mx-auto">
    ...
    {/* Monthly Sales/Units (YoY) — bar chart + table */}
    {cHF.some(h => h.units > 0) && <div className="bg-gray-900 rounded-xl p-4 mb-4 border border-gray-800"><h3 className="text-white font-semibold text-sm mb-2">Monthly Sales (YoY)</h3><div className="flex flex-col lg:flex-row gap-4"><div className="flex-1 min-w-0"><ResponsiveContainer width="100%" height={200}><BarChart data={unitsCh}><CartesianGrid strokeDasharray="3 3" stroke="#374151" /><XAxis dataKey="month" tick={{ fill: "#9ca3af", fontSize: 10 }} /><YAxis tick={{ fill: "#9ca3af", fontSize: 10 }} /><Tooltip {...TTP} /><Legend />{yrs.map(y => <Bar key={y} dataKey={"u_" + y} fill={YC[y] || "#6b7280"} opacity={0.85} radius={[2, 2, 0, 0]} name={"Units " + y} />)}</BarChart></ResponsiveContainer></div><div className="lg:w-72 overflow-x-auto"><table className="w-full text-xs"><thead><tr className="text-gray-500"><th className="py-1 px-1 text-left">Mo</th>{yrs.map(y => <th key={y} className="py-1 px-1 text-right" style={{ color: YC[y] || "#6b7280" }}>{y}</th>)}</tr></thead><tbody>{unitsCh.map((r, i) => <tr key={i} className={i % 2 === 0 ? "bg-gray-800/20" : ""}><td className="py-0.5 px-1 text-gray-300">{r.month}</td>{yrs.map(y => <td key={y} className="py-0.5 px-1 text-right text-white">{r["u_" + y] != null ? R(r["u_" + y]) : ""}</td>)}</tr>)}<tr className="border-t border-gray-700 font-semibold"><td className="py-1 px-1">Total</td>{yrs.map(y => <td key={y} className="py-1 px-1 text-right text-white">{R(uYTot[y])}</td>)}</tr></tbody></table></div></div></div>}
    {/* Pipeline */}
